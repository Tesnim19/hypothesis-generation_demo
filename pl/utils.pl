...
eval_pair(Index, gene(G), snp(S), 1) :- relevant_gene(Index, gene(G), snp(S)).
eval_pair(Index, gene(G), snp(S), 0) :- neg(relevant_gene(Index, gene(G), snp(S))).

cleanup :-
    retractall(relevant_gene(_, _, _)),
    retractall(neg(_)).

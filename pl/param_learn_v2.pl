...
load_train(Dir, Train) :-
  format(atom(DirectoryPath), '~w/train_models.txt', [Dir]),
  read_partition_file(DirectoryPath, Train).

load_test(Dir, Test) :-
  format(atom(DirectoryPath), '~w/test_models.txt', [Dir]),
...
run_param(_, _, [], [], [], [], [], [], []).
run_param(Dir, Program, [LPH|LPT],
         [AROCH|AROCT], [APRH|ARPT], [ROCH|ROCT], [PRH|PRT], [Res|RRest]) :- 
  load_train(Dir, TrainFold),
  load_test(Dir, TestFold),
  ...
  format(atom(ModelPath), '~w/models.pl', [Dir]),
  ...
  length(TrainFold, NTrain),length(TestFold, NTest),length(TrPos, TrNPos),length(TePos, TeNPos),
  length(TrNeg, TrNNeg),length(TeNeg, TeNNeg),
  ...
  retract_all([AllFRef]).
  % run_param(Dir, Program, LPT, AROCT, ARPT,
  %  ROCT, PRT, RRest).

run_param_learning(Dir, AUCROC, AUCPR, M_AUCROC, S_AUCROC, M_AUCPR, S_AUCPR, Threshold, Results) :-
  % numlist(0, NumFolds, Folds),
  format('Loading Program~n'),
  in(Program),
  format('Running parameter learning~n'),
  run_param(Dir, Program, LP, AUCROCs, AUCPRs, ROC, PR, Results), 
  AUCROCs = [AUCROC|_],
  AUCPRs = [AUCPR|_].
  % mean([AUCROC|_], M_AUCROC),
  % std_dev([AUCROC|_], S_AUCROC),
  % mean([AUCPR|_], M_AUCPR),
  % std_dev([AUCPR|_], S_AUCPR).

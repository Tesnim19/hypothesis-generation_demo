...
class GWASManifestParser:
    ...
    def _parse_row(self, row: Dict) -> Optional[Dict]:
        ...
        # Extract phenotype code - required field
        phenotype_code = (
        ...
        if not phenotype_code or phenotype_code.strip() == '':
            logger.debug(f"Skipping row with missing phenotype code: {row}")
            return None
    ...
    def validate_entries(self, entries: List[Dict]) -> tuple:
        ...
        for entry in entries:
            ...
            if not entry.get('phenotype_code'):
                entry_issues.append("Missing phenotype_code")
            elif entry['phenotype_code'] in seen_codes:
                entry_issues.append(f"Duplicate phenotype_code: {entry['phenotype_code']}")
            else:
                seen_codes.add(entry['phenotype_code'])
            
            if not entry.get('description'):
                entry_issues.append("Missing description")
            
            if not entry.get('display_name'):
                entry_issues.append("Missing display_name")
            ...
            if entry_issues:
                ...
                issues.append({
                    'phenotype_code': entry.get('phenotype_code', 'Unknown'),
                    'issues': entry_issues
                })
            else:

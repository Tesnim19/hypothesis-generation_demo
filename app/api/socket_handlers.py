"""Socket.IO event handlers (connect, subscribe_hypothesis, task_update)."""
...
async def handle_connect(sid: str, environ: dict, auth: dict | None = None) -> bool:
    token: str | None = auth.get("token") if auth and isinstance(auth, dict) else None
    if not token:
    ...
    try:
        ...
        if data.get("service") == "prefect":
            ...
            await sio.save_session(sid, {"user_id": str(data["user_id"]), "is_service": False})
            logger.info(f"[SIO] User {data['user_id']} connected: {sid}")
        return True
...
async def handle_subscribe(sid: str, data: dict | str) -> dict:
    try:
        ...
        hypotheses_handler = get_deps().get("hypotheses")
        hypothesis = (
        ...
        if is_complete:
            response_data.update({"status": "completed", "result": hypothesis, "progress": 100})
        else:

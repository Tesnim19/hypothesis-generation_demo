...
class GWASLibraryHandler(BaseHandler):
    ...
    def bulk_update_metadata(
        ...
        sample_size_key: str = 'sample_size',
        genome_build_key: str = 'genome_build',
    ) -> Dict[str, int]:
        """
        Update sample_size and genome_build for existing entries.
        
        Args:
            updates: List of dicts with file_id (or file_id_key), sample_size, genome_build
            file_id_key: Key for file identifier

        Returns:
            dict: {'updated': count, 'skipped': count, 'not_found': count}
        """
        updated_count = 0
        skipped_count = 0
        not_found_count = 0
        ...
        for row in updates:
            file_id = row.get(file_id_key) or row.get('filename') or row.get('file')
            if not file_id:
                skipped_count += 1
                continue
            
            update_data = {}
            if sample_size_key in row and row[sample_size_key] is not None:
                val = row[sample_size_key]
                if isinstance(val, (int, float)):
                    update_data['sample_size'] = int(val)
                elif isinstance(val, str) and val.strip():
                    try:
                        update_data['sample_size'] = int(float(val.replace(',', '')))
                    except (ValueError, TypeError):
                        pass
            if genome_build_key in row and row[genome_build_key]:
                build = str(row[genome_build_key]).strip()
                if build in ('GRCh37', 'GRCh38'):
                    update_data['genome_build'] = build
        ...
        logger.info(f"Bulk metadata update: {updated_count} updated, {skipped_count} skipped, {not_found_count} not found")

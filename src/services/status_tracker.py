...
class StatusTracker:
    ...
    def initialize(cls, task_handler, redis_url: Optional[str] = None) -> None:
        ...
        if redis_url:
            try:
                from services.state_cache import RedisStatusCache
    ...
    def _persist_and_clear(self, hypothesis_id: str) -> None:
        ...
        # Combine DB, Redis (survives worker restarts), and this process's memory
        ...
        deduplicated = {}
        for upd in combined:
            key = (upd["task"], upd["timestamp"])
            deduplicated[key] = upd
    ...
    def get_history(self, hypothesis_id: str):
        ...
        if cache is not None:
            try:
                ...
                logger.warning(
                    "StatusTracker: Redis read failed for {} – {}", hypothesis_id, exc
                )
        ...
        return sorted(deduplicated.values(), key=lambda x: x["timestamp"])

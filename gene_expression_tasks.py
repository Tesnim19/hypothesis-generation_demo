...
@task(log_prints=True)
def find_munged_gwas_file(db, user_id, project_id):
    ...
    try:
        ...
        analysis_results = db.get_analysis_results(user_id, project_id)
...
@task(log_prints=True)
def run_tissue_analysis_pipeline(db, current_user_id, project_id, causal_gene, hypothesis_id):
    """Run the complete tissue analysis pipeline using Prefect tasks"""
    
    # Find munged GWAS file from project analysis
    munged_file = find_munged_gwas_file.submit(db, current_user_id, project_id).result()
    if not munged_file:
    ...
    analysis_run_id = db.create_gene_expression_run(
    ...
    db.save_ldsc_results(analysis_run_id, ldsc_results_data)
    db.update_gene_expression_run_status(analysis_run_id, 'ldsc_completed')
...
@task(log_prints=True)
def wait_for_ldsc_completion(db, project_id, current_user_id, hypothesis_id, max_wait_time=600):
    ...
    while waited < max_wait_time:
        ...
        ldsc_status = db.check_gene_expression_status(project_id, current_user_id)
    ...
    # Get LDSC results
    ldsc_results = db.get_gene_expression_results(project_id=project_id, user_id=current_user_id)
...
def run_combined_ldsc_tissue_analysis(munged_file, output_dir, project_id, user_id):
    ...
    try:
        ...
        # Recreate database connection inside task to avoid serialization issues
        ...
        db.update_gene_expression_run_status(analysis_run_id, 'running')
        ...
        db.save_tissue_mappings(analysis_run_id, ontology_mapping_results)
        
        # Update status to completed
        db.update_gene_expression_run_status(analysis_run_id, 'ldsc_tissue_completed')
        ...
        db.update_analysis_state(project_id, user_id, analysis_summary)
        ...
        if analysis_run_id:
            db.update_gene_expression_run_status(analysis_run_id, 'failed')
        raise


@task(log_prints=True)
def run_background_ldsc_analysis(munged_file, output_dir, project_id, user_id, db):
